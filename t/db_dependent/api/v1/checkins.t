...
subtest 'sub-resource confirmation endpoints' => sub {
    ...
    warning_like {
        $t->post_ok( "//$userid:$password\@/api/v1/checkins/$checkin_id2/hold_cancellation" => json =>
    ...
    $t->post_ok( "//$userid:$password\@/api/v1/checkins/" . $checkin3->id . "/transfer_confirmation" )->status_is(201);
    ...
    $t->post_ok( "//$userid:$password\@/api/v1/checkins/" . $checkin4->id . "/transfer_cancellation" )->status_is(201);
...
subtest 'X-Koha-Module-Policy header on error paths' => sub {

    plan tests => 7;

    $schema->storage->txn_begin;

    # A librarian WITHOUT updatecharges/writeoff (so exempt_fine is denied),
    # but with circulate_remaining_permissions to reach the endpoint
    my $librarian = $builder->build_object( { class => 'Koha::Patrons', value => { flags => 2**1 } } );
    my $password  = 'thePassword123';
    $librarian->set_password( { password => $password, skip_validation => 1 } );
    my $userid = $librarian->userid;
    ...
    # The exempt_fine permission error path attaches the module policy header
    $t->post_ok(
        "//$userid:$password\@/api/v1/checkins" => json => {
            item_id     => $item->id,
            library_id  => $library->branchcode,
        ...
        ->status_is(403)
        ->json_is( '/error_code' => 'no_permission_for_exempt_fine' )
    ...
    is( $claims->{exempt_fine},     0, 'Policy JWT carries exempt_fine=0 for a user lacking writeoff' );

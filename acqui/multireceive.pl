...
my $ordernumber = $ordernumbers[0];

my $the_orders = Koha::Acquisition::Orders->search({ ordernumber => { -in => \@ordernumbers } });
...
my ( $template, $loggedinuser, $cookie, $userflags ) = get_template_and_user(
    {
        template_name   => "acqui/multireceive.tt",
        query           => $input,
        type            => "intranet",
        authnotrequired => 0,
...
$template->param('the_orders' => $the_orders);

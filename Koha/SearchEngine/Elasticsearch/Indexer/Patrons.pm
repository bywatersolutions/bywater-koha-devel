...
sub _build_document {
    ...
    # Use the API field mapping from Koha::Patron to translate DB column names
    ...
    for my $field (@direct_fields) {
        ...
        # Skip fields excluded from the API (mapped to undef)
        next if exists $api_field_map{$field} && !defined $api_field_map{$field};
...
sub update_mappings {
    ...
    try {
        ...
        Koha::Exception->throw(
            error =>
                "Unable to update mappings for index \"$index_name\". Reason: \"$reason\". Index needs recreation.",
        );
    };
...
sub get_elasticsearch_mappings {
    ...
    my $mappings_yaml = C4::Context->config('intranetdir') . '/admin/searchengine/elasticsearch/patron_mappings.yaml';
    my $config        = YAML::XS::LoadFile($mappings_yaml);
    my $fields        = $config->{patrons};

    my $field_config_yaml = C4::Context->config('intranetdir') . '/admin/searchengine/elasticsearch/field_config.yaml';
    ...
    for my $field_name ( keys %$fields ) {
        ...
        my $base_config = $field_config->{search}{$search_type} // $field_config->{search}{default};

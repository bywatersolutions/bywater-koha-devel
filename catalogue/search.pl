...
if  (
        ...
        ( (@limits>=1) || ($cgi->param("q")) || ($cgi->param('multibranchlimit')) || ($cgi->param('limit-yr')) ) 
    ) {
...
#        my %limit_hash = map { ($_ eq 'available') ? ($_ => 1) : split(':',$_) } @limits;
        my %limit_hash;
        foreach my $limit (@limits){
            if ( $limit eq 'available' ){
                $limit_hash{available} = 1;
            } else {
                ...
                if ( $index =~ /mc-/ ){
                    ...
                    $limit_hash{$index} = $value;
                }
            }
        };
        warn Data::Dumper::Dumper( @limits );
        warn Data::Dumper::Dumper( %limit_hash );
...
if($params->{'multibranchlimit'}) {
    ...
    push @limits, map { 'mc-homebranch:' . $_->branchcode } @libraries;
}
